...
const RootInstance = class extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		...
		if( !center ) {
			if( type && type["entity"] && type["entity"]["id"] ) {
				center = type["entity"]["id"];
			}
		}
					...
					>
						...
						>
							{ instance && instance["properties"] ? (
								<>
								{instance["properties"]["name"]} ({instance["properties"]["label"]})
								</>
							) : (
								<>
								</>	
...
export default withNavigation(withParams(connect(mapStateToProps, mapDispatchToProps)(RootInstance)));
